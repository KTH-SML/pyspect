...
class TVHJImpl(AxesImpl):
    ...
    def project_onto(self, vf, *axes, keepdims=False, union=True, complete=False):
        ...
        if union:
            out = vf.min(axis=tuple(dims), keepdims=keepdims)
        else:
            out = vf.max(axis=tuple(dims), keepdims=keepdims)
        if complete:
            out = out * np.ones([n for i, n in enumerate(self.shape) if i in axes])
        return out
